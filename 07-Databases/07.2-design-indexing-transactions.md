...
**BCNF, Boyce-Codd Normal Form.** A slightly stronger 3NF: for *every* functional dependency `X → Y`, `X` must be a superkey. 3NF allows a rare exception where a non-key attribute determines part of a candidate key; BCNF forbids it. In practice, designing to 3NF gets you BCNF the vast majority of the time. The cases that differ involve overlapping candidate keys — worth knowing they exist, rarely worth losing sleep over.
