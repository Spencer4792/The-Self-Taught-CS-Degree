...
This one ties straight to my resume: I added **secret scanning + dependency review** to a CI/CD pipeline. This chapter makes sure I can defend every word of that and go deeper.
...
4. **Scan**, security: dependency vulnerabilities, image CVEs, secret scanning, SAST. (This is *my* contribution area.)
...
Don't paste the giant workflow and pray. Build it incrementally and watch each piece go green. Here's how I'd actually do it, with the expected result at each step.
...
CI/CD is one of the most *visible* things you'll touch as an engineer, your PR can't merge until the checks are green, and your work isn't "done" until it deploys. Setting up or fixing pipelines is a frequent early-career task, and "walk me through your CI/CD" is a near-guaranteed interview question. Spencer's resume line (secret scanning + dependency review) is exactly the kind of concrete contribution interviewers love to dig into.
...
10. *"What did *you* add to a pipeline?"* (behavioral) → Spencer's answer: dependency review (blocks PRs introducing HIGH/CRITICAL CVEs), Gitleaks secret scanning (catches leaked credentials across history), and Trivy image scanning with `exit-code: "1"` (blocks deploys of vulnerable images).
...
5. In my workflow, name the three security gates I added and exactly what bad thing each one catches. Which step's `exit-code: "1"` blocks a deploy and why?
