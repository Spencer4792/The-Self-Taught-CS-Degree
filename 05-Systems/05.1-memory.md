...
As a flowchart, top to bottom:

```mermaid
flowchart TD
    High["HIGH addresses"] --> Stack["Stack (call frames, locals) - grows down"]
    Stack --> Free["Free space"]
    Free --> Heap["Heap (malloc'd memory) - grows up"]
    Heap --> BSS["BSS - uninitialized globals"]
    BSS --> Data["Data - initialized globals/statics"]
    Data --> Text["Text - code (read-only)"]
