...
> *Goal: stop treating memory as a magic infinite box. By the end you'll be able to draw a process's address space from memory, explain exactly what happens to the stack when a function is called, know what `malloc`/`free` really do, understand pointers in C well enough that Python's reference model stops being mysterious, and reason about why CPython needs *both* reference counting *and* a tracing GC. Plus: you'll *feel* the memory hierarchy in a timing experiment you run yourself.*

For most of a coding life, "memory" is a word you nod at. Variables held things; the things stayed. That works until it doesn't, until a recursion blows the stack, or a "fast" loop runs 10x slower than its twin for no obvious reason, or you leak memory in a *garbage-collected* language and get confused about how that's even possible. This chapter refuses to wave its hands. We're going to build the model from the silicon up.

---

## 🟢 Start here: the kitchen analogy

Before any of the technical stuff, here's a picture worth keeping in your head, and it's a kitchen.

Imagine you're cooking. Your **countertop** is small but everything on it is instantly within arm's reach, that's the **CPU's registers and L1 cache**. Your **pantry** across the room holds way more, but you have to walk to it, that's **RAM**. The **grocery store** down the road holds essentially unlimited food, but a trip takes forever, that's the **disk**. The whole art of cooking fast (and the whole art of fast programs) is the same: **keep what you're using close, and don't make pointless trips to the store.**

Now, two more kitchen ideas that map perfectly onto how a *program* uses memory:

- The **stack** is like a stack of plates by the sink. When a recipe calls a sub-recipe ("make the sauce"), you put a fresh plate on top with that sub-recipe's ingredients. When the sub-recipe finishes, you take its plate off and you're back to the recipe that called it. Plates go on and come off in strict last-on-first-off order. This is *automatic*, you never think about cleaning up; finishing a sub-recipe clears its plate.
...
Here are the rough latency numbers worth burning into your head. The absolute nanoseconds drift year to year, but the *ratios* are the point.
...
Here's a detail worth savoring: the stack and heap grow *toward each other* from opposite ends of the free space. In the bad old days, if they collided, you were in deep trouble. Today virtual memory and guard pages make the failure modes cleaner, but the mental image holds.
...
The mental model that makes this click: **Python always passes the reference by value.** The reference (pointer) is copied; the object is not. Mutation reaches the shared object; reassignment just repoints your private copy of the arrow.
...
Let's get our hands dirty. First, C pointer arithmetic, the thing that makes arrays and pointers feel like the same animal.
